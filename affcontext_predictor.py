...
if __name__ == '__main__':
    ...
    aff_context_model = affordancenet_context.get_aff_context_model(feature_extractor, rpn_model, cfg, base_anchors, mode="inference")
    # aff_context_model = affordancenet_context.get_aff_context_model_objectness(feature_extractor, rpn_model, cfg, base_anchors, mode="inference")
    # aff_context_model = affordancenet_context.get_aff_context_model_attention_task(feature_extractor, rpn_model, cfg, base_anchors, mode="inference")
    # aff_context_model = affordancenet_context.get_aff_context_model_auxiliary_task(feature_extractor, rpn_model, cfg, base_anchors, mode="inference")

    status = aff_context_model.load_weights(cfg.WEIGHTS_FILE, by_name=True)
    ...
    for i, image_data in enumerate(test_feed):
        ...
        # Evaluate results
        if cfg.EVALUATE:
            obj_stats, affordance_stats = eval_utils.update_stats_objecness(obj_stats, true_bboxes, true_labels, pred_bboxes, pred_labels,
